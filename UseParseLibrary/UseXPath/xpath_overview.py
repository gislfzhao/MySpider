...
selector = etree.parse('test3.html', parser=parser)
# print(etree.tostring(selector,encoding='utf-8'))
dynamics = selector.xpath('//body/div/div/div[@class="body"]')
del selector
print(len(dynamics))
# for dynamic in dynamics:
print(etree.tostring(dynamics[0], encoding='utf-8').decode('utf-8'))
a = etree.HTML(etree.tostring(dynamics[0], encoding='utf-8').decode('utf-8'), parser=parser)
item = a.xpath('//body/div[@class="body"]/div/div/div/div[1]/div//text()')
print(item)
a = list(map(lambda x: x.replace('\n', '').replace('\r', '').replace(' ', ''), item))
while '' in a:
    a.remove('')
print(' '.join(a))
print("测试")
# print(etree.tostring(item[0], encoding='utf-8').decode('utf-8'))
