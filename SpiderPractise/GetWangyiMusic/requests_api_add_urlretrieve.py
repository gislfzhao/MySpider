...
def urlretrieve(url, filename=None,reporthook=None, headers=None, allow_redirects=True, params=None):
    '''传入ID改变url，利用closing跟iter_content下载图片'''
    if not headers:
        headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.79 Safari/537.36'}
    # res = get(url, stream=True, headers=headers,params=params)
    # '''判断url是否是二进制流下载文件'''
    # if 'application/octet-stream' in res.headers.get('Content-Type'):
    with contextlib.closing(get(url, stream=True, headers=headers,
                                allow_redirects=allow_redirects, params=params)) as fp:  # 打开网页
